(function(){
  ...
  function selectEvent(e){
    ...
    if($('nextLocation'))$('nextLocation').textContent=e.location;
    if($('nextDate'))$('nextDate').textContent=e.date.toLocaleDateString('it-IT',{day:'numeric',month:'long',year:'numeric'});
    if($('markerDate'))$('markerDate').textContent=e.date.toLocaleDateString('it-IT',{day:'numeric',month:'long',year:'numeric'}).toUpperCase();
    if($('markerKind'))$('markerKind').textContent=e.kind;
    if($('detailType'))$('detailType').textContent=e.title.replace(/^Eclissi /,'');
    if($('detailVisibility'))$('detailVisibility').textContent=e.location;
    if($('detailStatus'))$('detailStatus').textContent='IN CAMMINO';
    const pct=updateCountdown(Date.now());
    updateScene(pct);
    document.querySelectorAll('.event-mini,.calendar-item').forEach(x=>x.classList.remove('selected'));
  }
  function renderSidebarV3(){
    const host=$('sidebarEvents'); if(!host)return;
    const list=future().slice(0,6);
    host.innerHTML=list.map((e,n)=>`<article class="event-mini" data-event-iso="${e.iso}" title="Seleziona ${e.title}"><div class="date">${e.label.replace(' ','<br>')}</div><div><h3>${e.title}</h3><p>${e.location}</p></div>${thumb(e)}<span class="arrow">›</span></article>`).join('');
  ...
  function renderCalendarV3(filter){
    ...
    const list=future().filter(e=>!filter||filter==='all'||e.type===filter);
  ...
  const originalCalendar=window.calendar;
  window.calendar=function(filter='all'){renderCalendarV3(filter)};
  const originalMini=window.miniEvents;
  window.miniEvents=function(){renderSidebarV3()};
  function init(){renderSidebarV3();renderCalendarV3('all');const b=$('calendarBtn2');if(b)b.addEventListener('click',openCalendar);}
